...
// --- DADOS ESTÁTICOS PARA SIMULAÇÃO VISUAL ---
...
const mockActivities = [
  {
    ...
    observation: "Artes finalizadas conforme o briefing. Seguem os links para aprovação e para o drive com os arquivos abertos.",
    links: [
      { label: "Acessar pasta no Drive", url: "#" },
    ],
  },
  {
    activityId: 102,
    ...
    observation: "Texto revisado e ajustado. Por favor, verificar se o tom de voz está adequado. A referência está no link abaixo.",
  ...
  },
  {
    ...
    observation: "Legendas criadas para os 5 posts da semana. Não há links para esta atividade.",
    links: [],
  },
];
...
const ActivitiesApproval = () => {
  ...
  const handleApprove = (activityId: number) => {
    alert(`Atividade #${activityId} APROVADA! (Ação simulada)`);
  };

  const handleReprove = (activityId: number) => {
    const reason = prompt(
      `Digite o motivo da reprovação e os ajustes necessários para a atividade #${activityId}:`
    );
    if (reason) {
      alert(
        `Atividade #${activityId} REPROVADA! (Ação simulada)\nMotivo: ${reason}`
      );
    }
  };

  const filterOptions: FilterOption[] = [
    { value: "recentes", label: "Mais Recentes", icon: "fa-solid fa-clock-rotate-left", baseColor: "bg-blue-500", textColor: "text-white" },
    { value: "antigas", label: "Mais Antigas", icon: "fa-solid fa-hourglass-end", baseColor: "bg-yellow-500", textColor: "text-zinc-900" },
    { value: "design", label: "Design", icon: "fa-solid fa-palette", baseColor: "bg-purple-500", textColor: "text-white" },
    { value: "social", label: "Social Media", icon: "fa-solid fa-share-nodes", baseColor: "bg-pink-500", textColor: "text-white" },
  ];
      ...
      <div className="flex flex-col md:flex-row justify-between md:items-center mb-4 gap-4">
        ...
        <div className="flex flex-col sm:flex-row gap-4 w-full md:w-auto">
          <ColoredButton
            ...
            onClick={() => navigate("/demandas/lista", { state: { previousRoute: "/demandas/atendente/aprovacao" } })}
          />
          <ColoredButton
            justify="justify-center"
            onClick={() => navigate("/demandas/nova", { state: { previousRoute: "/demandas/atendente/aprovacao" } })}
            color="customYellow"
            width="w-full sm:w-fit"
              ...
              <div className="flex flex-col gap-4">
                {mockActivities.length > 0 ? (
                    ...
                    >
                      <div className="flex justify-between items-center">
                        <h3
                          ...
                          onClick={() => navigate(`/demandas/${activity.demandId}`)}
                        >
                      ...
                      <div className="text-sm text-zinc-300">
                        Enviado por:{" "}
                        <span className="font-semibold text-white">{activity.collaboratorName}</span>
                        {" | "}
                        Equipe:{" "}
                        <span className="font-semibold text-white">{activity.collaboratorTeam}</span>
                      </div>
                  ...
                  <div className="text-center text-zinc-500 py-16">
                    ...
                    <p>Nenhuma atividade aguardando sua aprovação no momento.</p>
                  </div>
                )}
              </div>
            </StatusView>
