...
const DemandsScreen = () => {
  ...
  const allTasks: Task[] = useMemo(() => {
    ...
    return [
      {
        title: "Campanha Dia das Mães - Post 1",
        status: "concluída",
        indexCard: 1,
        prazo: format(addDays(today, -2), "yyyy-MM-dd"),
      },
      {
        title: "Reel Institucional - Cliente X",
        status: "em andamento",
        indexCard: 2,
        prazo: format(today, "yyyy-MM-dd"),
      },
      {
        title: "Criação de Logo - Startup Y",
        status: "não iniciada",
        indexCard: 3,
        prazo: format(today, "yyyy-MM-dd"),
      },
      {
        title: "Ajustes Landing Page - Cliente Z",
        status: "atrasada",
        indexCard: 4,
        prazo: format(addDays(today, -5), "yyyy-MM-dd"),
      },
      {
        title: "Copy para E-mail Mkt",
        status: "em andamento",
        indexCard: 5,
        prazo: format(addDays(today, 3), "yyyy-MM-dd"),
      },
      {
        title: "Design de Cartão de Visita",
        status: "concluída",
        indexCard: 6,
        prazo: format(addDays(today, 5), "yyyy-MM-dd"),
      },
  ...
  const { tasks, setTasks, activeCard, setActiveCard, onDrop } =
    useDragDrop(allTasks);
  ...
  const deadlines = useMemo(
    () => allTasks.map((task) => parseISO(task.prazo)),
    [allTasks]
  );
    ...
    <BaseScreen>
      ...
      <div className="flex flex-col w-full">
        <Motion>
          <Box
            title="Calendário"
            subtitle="Clique em uma data para ver as demandas com prazo no dia."
            width="w-full"
            height="h-fit"
          >
        ...
        <Motion>
          <Box
            ...
            subtitle="Verifique o progresso das demandas e modifique seu status."
            width="w-full"
            height="h-fit"
          >
